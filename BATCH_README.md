...
准确程度评分基于以下因素：
- **基础分数**: 50%
- **明确建议模式**: +20% (包含"Recommendation:"等明确表述)
- **技术分析深度**: +15% (MACD、RSI、SMA等技术指标数量)
- **风险管理讨论**: +10% (止损、波动性、风险评估等)
- **详细推理**: +10% (包含rationale、because等推理词汇)
- **多角度分析**: +5% (包含bull、bear、analyst等多重视角)
- **具体计划**: +5% (包含target、plan、strategy等具体执行计划)
- **内容详细度**: +5% (分析内容长度和详细程度)
